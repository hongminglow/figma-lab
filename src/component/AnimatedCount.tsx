...
type CountingNumberProps = React.ComponentPropsWithoutRef<"span"> & {
  number: number;
  fromNumber?: number;
  padStart?: boolean;
  inView?: boolean;
...
const CountingNumber = React.forwardRef<HTMLSpanElement, CountingNumberProps>(
  function CountingNumber(
    ...
    React.useEffect(() => {
      if (!isInView) return;
      // Ensure each update animates from the provided starting number.
      motionVal.set(fromNumber);
      motionVal.set(number);
    }, [isInView, number, fromNumber, motionVal]);
